...
def check_change_frequency_humidity(old_humidity, new_humidity, current_level) -> int:
    ...
    if old_humidity == new_humidity:
        return max( current_level + 1, 5) 
    else:
        return min( current_level - 1, 1)    
...
def main() -> None:
    ...
    for message in consumer:
        # Get the topic and set the new temperature or humidity
        topic = message.topic
        ...
        if topic == "temperatures":
            ...
            current_level = check_change_frequency_humidity(old_humidity, new_humidity, current_level)
